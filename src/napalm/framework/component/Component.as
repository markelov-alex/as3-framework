package napalm.framework.component
{
	 ...
	 * Component.
	 *
	 * Base component class for using with framework.
	 * 
	 * Use only following these steps:
	 * 1. Create instance (comp = new MyComponent()).
	 * 2. Set all public vars (comp.prop = "some").
	 * 3. Call initialize (comp.initialize(systemManager, assetManager)).
	 * 4. Set skinObject (comp.skinObject = getSkinChildByPath("closeButton")).
	 * Note: GUIComponent will create skinObject by itself.
	 * 
	 * It's important to set all public vars before initialize(), because it will use these vars, 
	 * and skinObject must be set after initialize(), because systemManager, assetManager and 
	 * assetManagerName are all set in initialize(). SkinObject must be added to stage.
	 * @author alex.panoptik@gmail.com
	 */
	public class Component extends SimpleComponent
	{
		...
		// Set like Component.isProductionMode = isProduction;
		public static var isProductionMode:Boolean = false;
		...
		override public final function set skinObject(value:Object):void
		{
			if (!_systemManager)
			{
				log.warn(Channel.COMPONENT, this, "Component " + this + " is not yet initialized, but skinObject is set!", isProductionMode ? new Error().getStackTrace() : "");
			}
